...
public class InvoiceCalculator : IInvoiceCalculator
{
    ...
    public InvoiceCalculator(SinaOTOSDbContext SinaOTOSDbContext, IHttpContextAccessor httpContextAccessor, IAppVersionService appVersionService)
    {
    ...
    {
        IList<VesselStoppage> vesselStoppages = await _SinaOTOSDbContext.VesselStoppages
        ...
        var voyage = await _SinaOTOSDbContext.Voyages.Include(c => c.Vessel).ThenInclude(c=>c.Type)
        ...
        foreach (var vesselStoppage in vesselStoppages)
        {
            ...
            VatTariff vatTariff = await _SinaOTOSDbContext.VatTariffs.AsNoTracking().OrderBy(c => c.EffectiveDate).LastAsync();
            if (vatTariff != null)
            {
    ...
    public async Task<BLMessage> Invoicing(PreInvoiceDto preInvoice)
    {
        ...
        try
        {
            await _SinaOTOSDbContext.Invoices.AddAsync(newInvoice);
            await _SinaOTOSDbContext.SaveChangesAsync();
        }
        catch (Exception ex)
        {
        ...
        foreach (var preInvoiceDetail in preInvoice.PreInvoiceDetails)
        {
            ...
            try
            {
                await _SinaOTOSDbContext.VesselStoppageInvoiceDetails.AddAsync(vesselStoppageInvoice);
                await _SinaOTOSDbContext.CleaningServiceInvoiceDetails.AddAsync(cleaningServiceInvoiceDetail);
            }
            catch (Exception ex)
            {
                result.Message = ex.Message;
    ...
    public async Task<PreLoadingDischargeInvoiceDto> CalculateLoadingDischargeInvoiceAsync(LoadingDischargeInvoiceDto loadingDischargeDto)
	{
		...
		try
        {
            loadingDischarge = await _SinaOTOSDbContext.LoadingDischarges
            ...
            loadingDischargeTariff = await _SinaOTOSDbContext.LoadingDischargeTariffs
        ...
        ulong craneTariff = 0, craneCostR = 0, inventoryTariff = 0, inventoryCostR = 0, discountAmount = 0;
        if (loadingDischarge.HasCrane)
        {
            ...
            craneCostR = (ulong)(craneTariff * loadingDischarge.Tonage);
		};
        ...
        ulong totalCostR = loadingDischargePriceR + craneCostR + inventoryCostR;
        if (loadingDischargeDto.DiscountPercent > 0)
        {
            discountAmount = totalCostR * loadingDischargeDto.DiscountPercent / 100;
        }
        byte vatPercent = (byte)(await _SinaOTOSDbContext.VatTariffs.OrderBy(c => c.EffectiveDate).LastAsync()).Rate;
        ...
        PreLoadingDischargeInvoiceDto result = new PreLoadingDischargeInvoiceDto(
            ...
            ShippingLineCompany: await _SinaOTOSDbContext.ShippingLines.SingleAsync(c => c.Id == loadingDischargeDto.shippingLineCompanyId),
            ...
            CraneTariff: craneTariff,
			CraneCostR: craneCostR,
    ...
    public async Task<BLMessage> LoadingDischargeInvoicing(PreLoadingDischargeInvoiceDto preLoadingDischargeInvoiceDto)
    {
        ...
        try
        {
            await _SinaOTOSDbContext.LoadingDischargeInvoices.AddAsync(newLoadingDischargeInvoice);
            await _SinaOTOSDbContext.SaveChangesAsync();
            result.State = true;
	...
	private string PrepareLoadingDischargeInvoiceNumber(out DateTime date)
    {
        ...
        invoiceNo.Append(shamsiDate.month.ToString().Length == 1 ? "0" + shamsiDate.month : shamsiDate.month);
        invoiceNo.Append('-');
        var lastInvoice = _SinaOTOSDbContext.LoadingDischargeInvoices.OrderBy(c => c.Id).LastOrDefault();
    ...
    {
        return await _SinaOTOSDbContext.Currencies.AsNoTracking().Where(c => c.Date <= vesselStoppage.ATA).OrderBy(c => c.Date).LastAsync();
    }

    #region Tariffs
    private async Task<VesselStoppageTariff> PrepareVesselStoppageTariff(DateTime ata)
    {
        return await _SinaOTOSDbContext.VesselStoppageTariffs.Include(c => c.VesselStoppageTariffDetails).AsNoTracking()
    ...
    {
        return await _SinaOTOSDbContext.CleaningServiceTariffs.Include(c => c.CleaningServiceTariffDetails).AsNoTracking()
            .Where(c => c.EffectiveDate <= ata).OrderBy(c => c.EffectiveDate).LastAsync();
    ...
    private decimal CalculateVesselStoppagePriceDollar(VesselStoppageTariffDetails vesselStoppageTariffDetail, Voyage voyage, uint dwellingHour)
    {
        ...
        {
            priceDVS = Math.Round(Convert.ToDecimal((dwellingHour * vesselStoppageTariffDetail.NormalPrice * voyage.Vessel?.GrossTonage) / 100), 4);
        }
        else
        {
    ...
    private double CalculateDiscount(Voyage voyage, DateTime ata,ref decimal priceDVS, ref decimal priceDCS)
    {
        ...
        {
            DiscountTariff? discountTariff =  _SinaOTOSDbContext.DiscountTariffs.AsNoTracking()
