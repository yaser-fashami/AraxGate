...
public class VesselStoppageService : IVesselStoppageService
{
    ...
    public VesselStoppageService(SinaOTOSDbContext SinaOTOSDbContext, IOperationRepository operationRepository)
    {
    ...
    private async Task<BLMessage> ValidateVesselStoppageAsync(VesselStoppage v)
    {
        ...
        if (v.Id != 0)
        {
            if (v.ATA != null)
            {
                if (await _SinaOTOSDbContext.VesselStoppages.Where(c => c.VoyageId == v.VoyageId && c.Id != v.Id).AnyAsync(c => c.ATA > v.ATA))
                {
                    result.Message = "There is at least a ATA after this ATA!";
                    result.State = false;
                    return result;
                }
                else if (await _SinaOTOSDbContext.VesselStoppages.Where(c => c.VoyageId == v.VoyageId && c.Id != v.Id).AnyAsync(c => c.ATA <= v.ATA && c.ATD == null))
                {
                    result.Message = "There is a InProcess stoppage before this ATA!";
                    result.State = false;
                    return result;
                }
                else if (v.ETA > v.ETD || v.ATA >= v.ATD || await _SinaOTOSDbContext.VesselStoppages.Where(c => c.VoyageId == v.VoyageId && c.Id != v.Id).AnyAsync(c => c.ATD >= v.ATD))
                {
            ...
            if (v.ATD != null)
            {
                if (await _SinaOTOSDbContext.VesselStoppages.Where(c => c.VoyageId == v.VoyageId && c.Id != v.Id).AnyAsync(c => c.ATD > v.ATD))
                {
                    result.Message = "There is at least a ATD after this date!";
                    result.State = false;
                    return result;
                }
                else if (await _SinaOTOSDbContext.VesselStoppages.Where(c => c.VoyageId == v.VoyageId && c.Id != v.Id).AnyAsync(c => c.ATA >= v.ATD))
                {
                    result.Message = "Your Dates is not valid!";
                    result.State = false;
                    return result;
        ...
        if (v.Id == 0)
        {
            if (await _SinaOTOSDbContext.VesselStoppages.Where(c => c.VoyageId == v.VoyageId).AnyAsync(c => c.ETA >= v.ETA || c.ATA >= v.ATA || c.ETD >= v.ETD || c.ATD >= v.ATD))
            {
            ...
            else if (await _SinaOTOSDbContext.VesselStoppages.Where(c => c.VoyageId == v.VoyageId).AnyAsync(c => c.ATA <= v.ATA && c.ATD == null))
            {
